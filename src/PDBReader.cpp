...
/// Extract a fixed-column field from a PDB line (1-based, inclusive),
...
//  PDB fixed-column layout (columns are 1-based, inclusive):
...
//  77-78  Element symbol (often absent in GROMACS output — derived from name)

std::vector<PDBAtom> parsePDB(const std::string& path) {
    std::ifstream f(path);
    if (!f)
        throw std::runtime_error("parsePDB: cannot open \"" + path + "\"");
    ...
    while (std::getline(f, line)) {
        ...
        // Assign mass via PeriodicTable (was wrongly called elementMass())
        a.mass = cactus::periodic::elementMassFromPDBName(a.name);
        atoms.push_back(a);
    }

    if (atoms.empty())
        throw std::runtime_error(
            "parsePDB: no ATOM/HETATM records found in \"" + path + "\"");
